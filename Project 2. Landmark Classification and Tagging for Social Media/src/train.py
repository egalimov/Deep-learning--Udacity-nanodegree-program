...
def valid_one_epoch(valid_dataloader, model, loss):
    ...
    with torch.no_grad():
        ...
        for batch_idx, (data, target) in tqdm(
            ...
            # Calculate average validations loss
...
def optimize2(data_loaders, model, optimizer, loss, n_epochs, save_path, interactive_tracking=False):
    # initialize tracker for minimum validation loss
    if interactive_tracking:
        liveloss = PlotLosses(outputs=[MatplotlibPlot(after_subplot=after_subplot)])
    else:
    ...
    # YOUR CODE HERE
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(  # =
        optimizer, "min", verbose=True, threshold=0.01, min_lr = 0.0001  # -
    )  # -
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        save_name = save_path+'_epoch_'+str(epoch)+'.pt'
        if valid_loss_min is None or (
                (valid_loss_min - valid_loss) / valid_loss_min > 0.01
        ):
            print(f"New minimum validation loss: {valid_loss:.6f}. Saving model ...")
...
def optimize3(data_loaders, model, optimizer, loss, n_epochs, save_path, interactive_tracking=False):
    # initialize tracker for minimum validation loss
    if interactive_tracking:
        liveloss = PlotLosses(outputs=[MatplotlibPlot(after_subplot=after_subplot)])
    else:
    ...
    # YOUR CODE HERE
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(  # =
        optimizer, "min", verbose=True, threshold=0.01, min_lr = 0.01  # -
    )  # -
...
def data_loaders():
    ...
    return get_data_loaders(batch_size=50, limit=200, valid_size=0.5, num_workers=1)
