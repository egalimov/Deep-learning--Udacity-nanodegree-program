...
def predictor_test(test_dataloader, model_reloaded, class_names, mean, std):
    """
    ...
    predictor_obj = Predictor(model_reloaded, class_names, mean, std)
    pred = []
    truth = []
    for x in tqdm(test_data, total=len(test_dataloader.dataset), leave=True, ncols=80):
        softmax = predictor_obj(x[0].unsqueeze(dim=0))
