...
def get_data_loaders(
    ...
    data_transforms = {
        "train": T.Compose(
            [T.Resize(256), T.RandomCrop(224),T.RandomHorizontalFlip(0.95),  
             ###
             T.RandomVerticalFlip(0.95),
...
#a = get_data_loaders(batch_size=32, valid_size=0.2, num_workers=-1, limit=-1)
#a2 = a['test']
...
#        sub.axis("off")
    ...
    dataiter  = iter(get_data_loaders(batch_size=32, valid_size=0.2, num_workers=1, limit=-1)['train']) # YOUR CODE HERE
...
def data_loaders():
    return get_data_loaders(batch_size=2, num_workers=1)
